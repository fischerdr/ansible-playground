...
description:
  - Run etcdctl defrag inside etcd pods (OpenShift) using oc rsh.
  - The module attempts to defragment non-leader members first and the leader last.
  - Intended to run inside Ansible Execution Environments (EEs); pass oc_binary if oc is not in PATH.
options:
  paas_cluster_name:
    description:
  ...
  etcd_cmd_timeout:
    description:
      - Timeout in seconds for etcdctl commands executed via oc rsh.
    required: false
    type: int
    default: 20
  member_list:
  ...
  oc_binary:
    description:
      - Path to the 'oc' binary to use. If omitted, module will try to discover oc on PATH inside the EE.
    required: false
    type: str
  kubeconfig:
    description:
      - Optional kubeconfig file path to pass to oc (e.g. --kubeconfig /path/to/kubeconfig).
    required: false
    type: str
...
class DefragController:
    def __init__(
        ...
        self.oc_binary = oc_binary or module.get_bin_path("oc", required=False)
        ...
        if not self.oc_binary:
            module.warn(
                "oc binary not found in EE PATH; provide 'oc_binary' parameter or include oc in the EE image."
            )
        if module.check_mode:
            module.exit_json(changed=False, results=[], skipped=True)

    # -------------------------
    # Utility: run oc commands
    # -------------------------
    def _oc_base(self) -> List[str]:
        cmd = [self.oc_binary] if self.oc_binary else ["oc"]
        if self.kubeconfig:
            cmd.extend(["--kubeconfig", self.kubeconfig])
        return cmd

    def _run_oc(
        self, args: List[str], cwd: Optional[str] = None, timeout: Optional[int] = None
    ) -> Tuple[int, str, str]:
        base = self._oc_base() + args
        cmd_str = " ".join([shlex.quote(p) for p in base])
        if self.debug:
            self.module.log(msg=f"Running command: {cmd_str}")
        rc, out, err = self.module.run_command(
            cmd_str, cwd=cwd, timeout=timeout, environ_update=None
        )
        return rc, out, err
    ...
    def _discover_etcd_pods(self) -> List[str]:
        args = ["get", "pods", "-n", "openshift-etcd", "-l", "app=etcd", "--no-headers"]
        rc, out, err = self._run_oc(args, timeout=30)
        if rc != 0:
            self.module.warn(f"oc get pods returned rc={rc}; stderr={err.strip()}")
            return []
        pods = []
        ...
        if self.debug:
            self.module.log(msg=f"Discovered etcd pods: {pods}")
        return pods

    # -------------------------
    # rsh -> etcdctl helpers
    # -------------------------
    def _rsh_and_run_etcdctl(
        self, pod: str, etcdctl_cmd: str, timeout_offset: int = 10
    ) -> Tuple[int, str, str]:
        if not self.oc_binary:
            return 127, "", "oc binary not available"

        # Timeout wrapper inside pod to guard remote command length
        inner = f"timeout {self.etcd_cmd_timeout}s {etcdctl_cmd}"
        args = ["rsh", "-n", "openshift-etcd", pod, "--", inner]
        rc, out, err = self._run_oc(
            args, timeout=self.etcd_cmd_timeout + timeout_offset
        )
        if self.debug:
            self.module.log(
                msg=f"oc rsh -> pod={pod}, cmd={etcdctl_cmd}, rc={rc}, out_len={len(out)}, err_len={len(err)}"
            )
        return rc, out, err
    ...
    def _get_member_list_via_etcdctl(self, pod: str) -> List[Dict]:
        ...
        """
        rc, out, err = self._rsh_and_run_etcdctl(
            pod, "etcdctl member list --write-out=json"
        )
        if rc != 0:
    ...
    def _get_endpoint_status_json(self, pod: str) -> List[Dict]:
        ...
        """
        rc, out, err = self._rsh_and_run_etcdctl(
            pod, "etcdctl endpoint status --write-out=json"
        )
        if rc != 0:
            self.module.warn(
    ...
    def defrag_member(self, pod: str) -> dict:
        ...
        """
        try:
            rc_check, out_check, err_check = self._rsh_and_run_etcdctl(
                pod, "etcdctl endpoint status --write-out=table"
            )
            if rc_check != 0:
                res = {
                    "member": pod,
            ...
            rc, out, err = self._rsh_and_run_etcdctl(pod, "etcdctl defrag")
            if rc != 0:
                res = {
                    "member": pod,
                    "changed": False,
...
def main():
    argument_spec = dict(
        ...
        oc_binary=dict(type="str", required=False, default=None),
    ...
    controller = DefragController(
        ...
        oc_binary=module.params["oc_binary"],
