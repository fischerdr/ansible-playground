...
class DefragController(object):
  ...
  def run(self, cmd):
    oc_cmd = "timeout {} /root/clusters/{}/oc --kubeconfig=/root/clusters/{}/install/auth/kubeconfig ".format(self.etcd_cmd_timeout, self.paas_cluster_name, self.paas_cluster_name) + cmd
    return subprocess.check_output(oc_cmd, cwd="/root/clusters/{}/".format(self.paas_cluster_name), shell=True)

  def defrag_etcd_member(self, member):
    if not self.run("rsh -n openshift-etcd {} bash -c 'unset ETCDCTL_ENDPOINTS && etcdctl --endpoints=https://localhost:2379 defrag --command-timeout={}s' 2>/dev/null".format(member, self.etcd_cmd_timeout)):
  ...
  def defrag_non_leader(self, member):
    leader_id = self.run("rsh -n openshift-etcd {} etcdctl endpoint status --command-timeout={}s 2>/dev/null | tr -d '' | tr -d ',' | awk '$6 == \"true\" {{print $2}}'".format(member, self.etcd_cmd_timeout))
    leader_name = self.run("rsh -n openshift-etcd {} etcdctl member list --command-timeout={}s 2>/dev/null  | grep {} | awk '{{print $3}}' | tr -d ','".format(member, self.etcd_cmd_timeout, leader_id.strip('\n')))
  ...
  def defrag_etcd_db(self):
    ...
    if len(self.member_list) > 1:
      ...
      etcd_pod_list = self.run("get pods -n openshift-etcd -l app=etcd --no-headers | awk '{print $1}'")
    ...
    while len(completed_members_list) < non_leader_baseline:
      if retry_count > member_count:
        raise Exception("ETCD defragmentation exceeded maximum retries. Please check ETCD health.")
    ...
    if len(completed_members_list) == member_count:
      ...
      raise Exception("Defrag not done for all members. Completed members: {}".format(completed_members_list))
