                    ...
                    if any(
                        ...
                        for op in ["==", ">=", "<=", "!=", "~=", ">", "<"]:
                            if op in line:
                                ...
                                requirements[package.strip().lower()] = (
                                    f"{op}{version.strip()}"
                                )
                                break
                    else:
...
def find_overlaps(
    main_reqs: Dict[str, str], collection_reqs: Dict[str, str]
) -> Dict[str, Dict]:
    ...
    for package in set(main_reqs.keys()) & set(collection_reqs.keys()):
        overlaps[package] = {
            ...
            "compatible": analyze_compatibility(
                main_reqs[package], collection_reqs[package]
            ),
        }
